/**
 * @module home-client
 * @description Main home/landing page client component for the StockHuntr application
 *
 * PURPOSE:
 ...
 *
 * EXPORTS:
 * - default: HomeClient - Main page component that conditionally renders dashboard or marketing content
 * - TopSignal (interface) - Shape of a top LONG/SHORT signal, shared with the server page
 *
 * CLAUDE NOTES:
 * - Client component using Next.js 13+ App Router ('use client' directive)
 * - Receives server-fetched initial data (latest filings + top signals) from app/page.tsx so the
 *   first server-rendered HTTP response contains full page content for SEO and AI crawlers
...
interface RecentFiling {
  ...
  companyName: string;
  formType: string;
  filingType?: string; // API returns this field name
  filedAt: string;
...
interface HomeClientProps {
  initialFilings: LatestFilingsResult | null;
  initialTopSignals: TopSignal[];
}

export default function HomeClient({ initialFilings, initialTopSignals }: HomeClientProps) {
  const [user, setUser] = useState<User | null>(null);
  // When server-fetched initial data is present, render the full page immediately (no spinner)
  // so the first server-rendered response contains real content for crawlers.
  const [loading, setLoading] = useState(!initialFilings);
  const [watchlist, setWatchlist] = useState<WatchlistItem[]>([]);
  const [recentFilings, setRecentFilings] = useState<RecentFiling[]>(
    (initialFilings?.filings ?? []) as unknown as RecentFiling[]
  );
