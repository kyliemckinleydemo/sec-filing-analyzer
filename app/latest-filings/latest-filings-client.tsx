/**
 * @module app/latest-filings/latest-filings-client
 * @description Client component displaying paginated SEC filings with company search, filtering by ticker/filing type, and routing to individual filing analysis. Server-rendered with initialData for SEO.
 *
 * PURPOSE:
 * - Render server-provided initial filings list so the first HTTP response contains full content
 ...
 * - @/lib/filings-server - Type-only import of LatestFilingsResult for the initialData prop
 *
 * EXPORTS:
 * - LatestFilingsClient (component) - Client component for /latest-filings route, receives initialData from server page
 *
 * PATTERNS:
 * - Initialize state from initialData so the server render outputs the full filings list (no loading skeleton)
 * - Read ticker filter from window.location.search on mount (NOT useSearchParams, which would force a
 *   Suspense fallback into the statically generated HTML and hide content from crawlers)
 ...
 *
 * CLAUDE NOTES:
 * - Skips the initial client-side fetch when filters match the server-rendered default view (no ticker, all types)
 * - Listens for popstate so browser back/forward still re-applies the ticker URL param
...
import type { LatestFilingsResult } from '@/lib/filings-server';
...
export default function LatestFilingsClient({ initialData }: { initialData: LatestFilingsResult }) {
  const [filings, setFilings] = useState<LatestFiling[]>(initialData.filings);
  ...
  // rendered with full content (useSearchParams forces a Suspense fallback into the
  // prerendered HTML). popstate keeps back/forward navigation in sync.
  useEffect(() => {
    const syncFromUrl = () => {
  ...
  useEffect(() => {
    ...
    // On first run: if filters match the server-rendered default view, the
    // initialData already covers it - no client refetch needed.
    if (!hasHandledInitialFilters.current) {
      hasHandledInitialFilters.current = true;
      if (!tickerFilter && filingTypeFilter === 'all') return;
    }
